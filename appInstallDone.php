    ...
    <script>
      $(function() {
          ...
          $('.form-install').submit(function(e) {
              ...
              $.post('api/usuario/install.php', { nome: $('#nome').val(), usuario: usuario, senha: senha, email: $('#email').val(), rand: Math.random()}, function(data) {
                  $('.btn-install').html('<img src="dist/img/rings.svg" class="loader-svg">').fadeTo(fade, 1);

                  switch(data) {
                  case 'true':
                      Toast.fire({icon: 'success',title: 'Usu&aacute;rio criado.'}).then((result) => {
